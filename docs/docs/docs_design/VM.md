...
In which the general design of the V11 VM is explained.

## Value Representation

The VM uses a class called `V11Value` to encapsulate JS values efficiently,
...
and `StringPrimitive *` can be stored in `V11Value` to make JS String values.
...
with the exception of values stored as `PinnedV11Value`
(mainly used for global objects stored in the `Runtime` itself).
The garbage collector is precise
(it knows what `V11Value`s are valid pointers to objects in the JS heap).
...
invalidating `V11Value`s, so there are a couple "scoped" value classes.
...
use the handles instead of passing raw `V11Value` between functions.

A `GCScope` is used to keep track of all the current `V11Value` handles.
...
The `GCScope` is used to internally generate `PinnedV11Value`s,
...
The V11VM provides a REPL in `bin/v11-repl`,
